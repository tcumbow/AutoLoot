...
local function OnEventInteractableTargetChanged()
	...
	if blocked or additionalInfo==2 or IsMounted() or (action==LastAction and target==LastTarget and target~="Grapple Point") then
		return
	end
	...
	if 	(
			...
			or (action=="Use" and (target=="Grapple Point" or target=="Chest" or target=="Treasure Chest" or target=="Giant Clam" or target=="Skyshard"))
